    ...
    public function execute()
    {
		if ($this->session->isLoggedIn()) {
				...
				if($shoppinglistid) {
					...
					$sql = "DELETE FROM shopping_list_item WHERE shopping_list_id = " . $shoppinglistid;
					$connection->query($sql);
					
					$sql = "DELETE FROM shopping_list WHERE id = " . $shoppinglistid;
					$connection->query($sql);
					
					$this->session->setShoppinglistId('');
					
					$this->messageManager->addSuccess( __(' Shoppinglist Deleted Successfully.') );
